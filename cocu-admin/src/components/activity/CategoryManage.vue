<template>
    <div>
        <div style="margin-bottom: 5px;">
            <el-input v-model="name" placeholder="Please enter category name" suffix-icon="el-icon-search"
        ...
        <el-table :data="tableData" :header-cell-style="{background: '#f2f5fc', color: '#555'}" border>
            ...
            <el-table-column prop="name" label="Type Name" width="250">
            </el-table-column>
            <el-table-column prop="description" label="Description">
            </el-table-column>

            <el-table-column prop="operate" label="Operation" width="200">
                <template slot-scope="scope">
        ...
        <el-dialog
                title="Operate Category Information"
...
export default {
    ...
    methods: {
        loadList(){
            ...
            this.$axios.post(this.$httpUrl + '/category/listPage', {
                pageSize: size,
                pageNum: num,
                data: {
                    // search category input name
                    name: this.name
                }
            }).then(res => {
                console.log(res)
        ...
        handleAvatarSuccess(res) {
            // Handle the avatar upload success, update category's avatar with the new URL
            let that = this
            if(res.code == 200){
                //this.imageUrl = URL.createObjectURL(file.raw);
                that.form.avatar = res.data.url
            }else{
                this.$message.error(`category avatar upload failed:${res.message}`)
            }
        ...
        saveClubType(){
            this.$axios.post(this.$httpUrl + '/category/save', this.form).then(res => {
                console.log(res)
                if(res.code == 200){
                    ...
                    this.$message({
                        showClose: true,
                        message: 'Oops, category create failed.',
                        type: 'error'
                    });
                }
            })
        ...
        editClubType(){
            this.$axios.post(this.$httpUrl + '/category/update', this.form).then(res => {
                //console.log(res)
                if(res.code == 200){
                    this.centerDialogVisible = false;
                    this.$message({
                        showClose: true,
                        message: 'Congrats, update category successfully.',
                        type: 'success'
                    });
                    this.loadList()
                }else{
                    this.$message({
                        showClose: true,
                        message: 'Oops, category update failed.',
                        type: 'error'
                    });
                }
            })
        ...
        del(id){
            console.log(id)
            this.$axios.get(this.$httpUrl + '/category/delete?id=' + id).then(res => {
                //console.log(res)
                if(res.code == 200){
                    this.$message({
                        showClose: true,
                        message: 'Congrats, delete category successfully.',
                        type: 'success'
                    });
                    this.loadList()
                }else{
                    this.$message({
                        showClose: true,
                        message: 'Oops, category delete failed.',
                        type: 'error'
                    });
                }
            })
