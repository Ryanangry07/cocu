<template>
    <div>
        <div style="margin-bottom: 5px;">
            <el-input v-model="name" placeholder="Please enter tag name" suffix-icon="el-icon-search"
...
export default {
    ...
    data() {
        return {
            ...
            rules: {
                ...
                description: [
                    {required: true, message: 'Please enter tag description', trigger: 'blur'},
                ]
            }
        }
    },
    methods: {
        loadList(){
            ...
            this.$axios.post(this.$httpUrl + '/tag/listPage', {
                pageSize: size,
                pageNum: num,
                data: {
                    // search tag input name
                    name: this.name
                }
            }).then(res => {
                console.log(res)
        ...
        handleAvatarSuccess(res) {
            // Handle the avatar upload success, update tag's avatar with the new URL
            let that = this
            if(res.code == 200){
                //this.imageUrl = URL.createObjectURL(file.raw);
                that.form.avatar = res.data.url
            }else{
                this.$message.error(`tag avatar upload failed:${res.message}`)
            }
        ...
        saveClubType(){
            this.$axios.post(this.$httpUrl + '/tag/save', this.form).then(res => {
                console.log(res)
                if(res.code == 200){
                    ...
                    this.$message({
                        showClose: true,
                        message: 'Oops, tag create failed.',
                        type: 'error'
                    });
                }
            })
        ...
        editClubType(){
            this.$axios.post(this.$httpUrl + '/tag/update', this.form).then(res => {
                //console.log(res)
                if(res.code == 200){
                    this.centerDialogVisible = false;
                    this.$message({
                        showClose: true,
                        message: 'Congrats, update tag successfully.',
                        type: 'success'
                    });
                    this.loadList()
                }else{
                    this.$message({
                        showClose: true,
                        message: 'Oops, tag update failed.',
                        type: 'error'
                    });
                }
            })
        ...
        del(id){
            console.log(id)
            this.$axios.get(this.$httpUrl + '/tag/delete?id=' + id).then(res => {
                //console.log(res)
                if(res.code == 200){
                    this.$message({
                        showClose: true,
                        message: 'Congrats, delete tag successfully.',
                        type: 'success'
                    });
                    this.loadList()
                }else{
                    this.$message({
                        showClose: true,
                        message: 'Oops, tag delete failed.',
                        type: 'error'
                    });
                }
            })
